...
import random
from threading import active_count, Threadimport random
import string
import pathlib
import requests, os, threading, sys, time, random, ctypes, webbrowser,re, hashlib, datetime, os.path, httpx, sys, gratient
import colorama
...
def gen():
    ...
    if auau == "null:null":
        ...
        req = requests.get(f'https://hypixel.net', headers=Header, proxies=dict(http=f"http://{auau}", https=f"http://{auau}"))
